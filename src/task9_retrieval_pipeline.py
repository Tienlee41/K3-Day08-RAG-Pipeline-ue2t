...
def retrieve(
    ...
    # If dense retrieval is unavailable (for example before Chroma is
    # installed), lexical evidence is still a valid local path. Do not throw
    # it away in favour of broad whole-document PageIndex sections. PageIndex
    # is only a fallback when neither dense nor sparse retrieval produced
    # candidates, or when dense is weak and sparse is empty.
    if best_dense_score < threshold and not sparse_results:
